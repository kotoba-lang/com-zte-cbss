(ns zte_cbss.main
  "Kotodama WASM entrypoint for the Zte_cbss clean-room actor (L4) — Clojure port.
  ...
  facts (`zte_cbss.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zte_cbss")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zte_cbss.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zte_cbss-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zte_cbss-compat/ready)
